...
def fetch_github(kenni, url, term):
    t = urlib2parse.quote(term)
    if '%' in term:
        t = urlib2parse.quote(term.replace('%', ''))

    request = urlib2request.Request(url % t, headers=DEFAULT_HEADER)

    try:
        content = json.loads(urlib2request.urlopen(request).read())
        return content
