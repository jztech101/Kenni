...
class kenniURLopener(urlib2request.FancyURLopener):
    version = USER_AGENT

urlib2request._urlopener = kenniURLopener()
...
def random_quote(kenni, cat):
    ...
    while(True):
        try:
            ...
            content = json.loads(urlib2request.urlopen(cat_url).read())
    ...
    try:
        ...
        content = json.loads(urlib2request.urlopen(page_url).read())
        ...
        while quote == None and num_tries < MAX_TRIES:
            ...
            content = json.loads(urlib2request.urlopen(section_url).read())
